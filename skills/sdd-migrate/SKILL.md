...
4. Land the instance exactly as the sdd-setup skill's `## Land an instance` and `## Wire agent context` state it: the profile, the preview against the assess report's collisions, the apply, the documentation section in `AGENTS.md`. Nothing of that sequence is restated here. Then carry the inherited corpus as the sdd-setup skill's `## Carry an inherited corpus` states, choosing the verb by the two files under `.spec-driven-docs/` so the operator faces no choice: `chapter-size-debt.txt` present routes to `sdd debt migrate`, neither file present routes to `sdd debt baseline`, both present is a failure whose recovery is `sdd debt migrate --apply`, and `debt.yaml` alone takes neither, so a remaining violation is fixed or `sdd debt tighten` lowers a ceiling with slack. Preview first, then apply, before the first commit.
