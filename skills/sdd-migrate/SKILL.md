...
- `needs-decision` is the operator's call, asked with `AskUserQuestion` and the report's evidence before any plan claims to know what those documents are. The evidence is the documents the report found outside every recognized home.
- An instance routes by its status report, whatever the verdict says, and `alignment` decides first: `instance-newer` stops until a matching `sdd` is installed, because no verb below is safe from an older binary. `binary-newer` takes the sdd-setup skill's `## Upgrade`, which owns the dry run, the apply, the atomic conflict refusal, and cleanup of empty directories left by removed managed files.
- An `aligned` instance routes by the report's two counts, in this order, and the branches do not overlap: `failures` above zero takes the sdd-setup skill's `## Verify`. Those failures are managed drift, a missing adopted file, a broken integration block, or a duplicate rule ID. There this skill fixes what `sdd verify --target .` cites, smallest first, with managed files reverted or reconciled as `## Upgrade` states. Then `adopted_drift` above zero is the project's own reconciliation, `DRIFT` lines this skill never reverts. Only both at zero is reported and stops, because there is nothing to migrate.
...
2. Classify each entry's facts by the placement chapter's decision procedure, `sdd method placement`. One source often splits across several fates. List each fate under its entry rather than averaging them.
...
4. Land the instance exactly as the sdd-setup skill's `## Land an instance` and `## Wire agent context` state it: the profile, the preview against the assess report's collisions, the apply, the documentation section in the root agent digest. Nothing of that sequence is restated here. Then carry the inherited corpus as the sdd-setup skill's `## Carry an inherited corpus` states, choosing the verb by the two debt files `sdd status --target . --json` reports so the operator faces no choice: the legacy list `paths.active.destinations.legacy_debt` names present routes to `sdd debt migrate`, neither file present routes to `sdd debt baseline`, both present is a failure whose recovery is `sdd debt migrate --apply`, and the debt file `paths.active.destinations.debt` names alone takes neither, so a remaining violation is fixed or `sdd debt tighten` lowers a ceiling with slack. Preview first, then apply, before the first commit.
5. Run the chapter's loop in verified batches: re-checksum, rewrite into the owning zone, verify the destinations, retire the source only under the chapter's four conditions, record completion last. Each destination is authored as the sdd-write-docs skill's `## Before writing` states: the adopted spec for its domain loaded, the matching template taken only where one exists. A plain reference page has none and follows its zone's format and placement rules. The writing style is not a sweep criterion. A document already in the corpus converts the next time somebody edits it, no gate reports documents that have not converted, and a writing-style sweep is not part of a migration.
...
A numbered document, `01-intro.md` or `ADR-0007-thing.md`, is renamed to a slug drawn from its subject as part of the migration, not as a follow-up, because a number is an identity two branches can both claim. The rename repairs every inbound link in the same change, and a link check proves it. Where the corpus has a reading order, that order moves into the directory's `README.md` as prose before the numbers come off, so nothing is lost between the two steps. It stays prose: no code reads it, and the migration adds none that does. No gate holds an unprefixed name, so an agent that waits for one to report the omission never does the work. `adr-filename-shape` and `ki-filename-shape` judge the kind-prefixed set alone.
...
- Prefer an sdd verb over hand-editing anything the instance manifest owns. Everything under the instance directory `paths.active.destinations.instance_dir` names belongs to the canon.
