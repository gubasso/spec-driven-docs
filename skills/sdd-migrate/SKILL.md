...
- An instance routes by its status report, whatever the verdict says, and `alignment` decides first: `instance-newer` stops until a matching `sdd` is installed, because no verb below is safe from an older binary; `binary-newer` takes the sdd-setup skill's `## Upgrade`, which owns the dry run, the apply, and the atomic conflict refusal; `aligned` with nothing failing is reported and stops, because there is nothing to migrate.
- Drift on an aligned instance has two owners, per the sdd-setup skill's `## Verify`: managed drift is a `FAIL` the canon owns, reverted or reconciled as `## Upgrade` states; adopted drift is a `DRIFT` the instance owns, reconciled by the project and never reverted by this skill. Fix what `sdd verify --target .` cites, smallest first, by that ownership.
