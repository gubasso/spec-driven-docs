...
6. For a setup or migration task at a target with no instance, classify it: `sdd assess --target . --json` reads the evidence — documentation roots, the document inventory, methodology markers, collisions — and answers `greenfield`, `brownfield`, or `needs-decision`. The verdict routes the plan: greenfield lands an instance, brownfield loads the migration skill, and needs-decision is the operator's call, asked with the evidence in front of them. A target already carrying an instance routes by its status report — verify or upgrade — not by classification, whose corpus verdict describes every healthy instance.
