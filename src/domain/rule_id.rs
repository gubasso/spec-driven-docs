...
rule_ids! {
    ...
    PartialFailureReportsWhatItCompleted => "staging:a-partial-failure-reports-what-it-completed",
    StageCarriesTheWholeCandidate => "staging:a-stage-carries-the-whole-candidate",
    StagePersistsUntilItIsCleaned => "staging:a-stage-persists-until-it-is-cleaned",
    StageWritesOnlyTheStage => "staging:a-stage-writes-only-the-stage",
    UserScopeReceiptSharesNoEngine => "staging:a-user-scope-receipt-shares-no-engine",
    UnattributedCollisionRefusesFirst => "staging:an-unattributed-collision-refuses-first",
    EveryPathStaysUnderTheTarget => "staging:every-path-stays-under-the-target",
    OneDocumentationRootServesTheRun => "staging:one-documentation-root-serves-the-run",
    OneWriterHoldsTheTarget => "staging:one-writer-holds-the-target",
    OwnerOnlyStateIsCheckedOnLinux => "staging:owner-only-state-is-checked-on-linux",
    ProductionReadsNoStagedByte => "staging:production-reads-no-staged-byte",
    ManifestAttributesEveryRefresh => "staging:the-manifest-attributes-every-refresh",
    ManifestIsWrittenLast => "staging:the-manifest-is-written-last",
    OperatorOwnsAcquisition => "staging:the-operator-owns-acquisition",
