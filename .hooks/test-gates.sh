...
accept 'md spec shape' markdownlint-cli2 --config "$hooks/../.markdownlint/spec.markdownlint-cli2.jsonc" "$spec"
bad_spec="$tmp/bad-spec.md"
sed 's/## Purpose/## purpose/' "$spec" >"$bad_spec"
reject 'md spec wrong case' markdownlint-cli2 --config "$hooks/../.markdownlint/spec.markdownlint-cli2.jsonc" "$bad_spec"
adr="$tmp/ADR-sample.md"
printf '# Sample\n\n## Context and Problem Statement\n\nContext.\n\n## Considered Options\n\n- x — chosen.\n\n## Decision Outcome\n\nChosen.\n\n## Consequences\n\n- Good: small\n\n## Status\n\nAccepted\n' >"$adr"
accept 'md adr shape' markdownlint-cli2 --config "$hooks/../.markdownlint/adr.markdownlint-cli2.jsonc" "$adr"
