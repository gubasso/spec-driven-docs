...
Modify keeps the ID when the rule is still about the same thing, so every commit and comment citing it still resolves. It gets a new ID when the subject changed, which makes it an add and a remove.

Remove deletes the requirement outright. No strikethrough, no `deprecated` marker, no note saying the rule used to exist. The commit holds the diff and a decision record holds the reasoning.
...
A rule lands after the material it binds. Applying it to what predates it is ordinary work, not a precondition for stating it.
...
`.draft/` at the project root is the workshop, and it is gitignored. Discovery notes, raw outlines, copied issue text, and half-shaped arguments live there.
...
If a draft tangles exploration with something the project is already working under, split it: the binding part goes to the plan zone under version control, and the rest stays in `.draft/` until it resolves.
...
Some documents hold facts that expire without any local change: benchmark results, vendor pricing, model and tool rosters, external API shapes, security advisories, dependency lifecycle dates, platform support matrices.

- A document holding a fact that depends on an external source MUST have an entry in the tracking registry.

The registry is machine-readable, one entry per artifact. It records enough for a human or an agent to know what is stale and how to check it.
...
Revalidation is a scan for overdue entries, a re-fetch from the authoritative source, an update to the artifact, and a bump to `last_checked`. Overdue is deterministic and belongs to tooling. Deciding what the new truth is requires judgment and belongs to a person.

- An agent that cannot re-verify a claim MUST report it rather than overwrite it.

Do not track stable conceptual documents because they are old. A rule does not expire on a timer. A decision changes or removes it.

## External-system bugs

A bug in a dependency, platform, or service that the project must work around is recorded once, in reference. The record carries the workaround and the condition that retires it.
...
A record that only names the defect, as in "the formatter moves the marker", is unfalsifiable to everyone but its author. The next reader re-derives the mechanism from scratch. The shape that avoids that names the two or three moving parts, runs one concrete case, and shows what each step leaves behind: the input, the command that acts on it, and the state after it. As a result, the wrong step is visible rather than asserted. Close with the cause in one line, the reading a triager reaches for first and why it is wrong, and the smallest fix. Quote captured output verbatim, tied to the step it belongs to.
...
Filing is where the record leaves the repository. A record written only for the project is written a second time in the tracker, under whatever deadline made the bug worth filing. The two drift from that moment on. A `## Report` section that is the filed text, title on its own line and body in the tracker's markup, makes the filing a paste. It also keeps the repository holding what was actually said upstream.
...
The two axes are separate because they move independently. A masked workaround in the tree says nothing about whether the evidence behind it is ticket-ready. A filed case is handled the same way the day before the fix ships as the day after. One word carrying both answers neither question by itself.

The pair that earns the vocabulary is `mitigated` against `masked`, because they look alike in the diff and retire oppositely. A readiness check that waits for a service to report healthy is part of the design. It outlives the bug that exposed the need for it. A fixed sleep that hides the same race is a mask, and it leaves when the bug does. One case can carry both.

When the condition is met, remove the workaround and delete the record in the same change. The record existed to describe a live constraint. A constraint that lifted leaves no trace in prose.
...
- OpenSpec, on delta descriptions stating what changes without restating the unchanged: <https://github.com/Fission-AI/OpenSpec/blob/main/docs/concepts.md>
