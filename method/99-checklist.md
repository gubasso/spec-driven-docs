...
- [ ] Where the change touched both a spec and a decision record, the spec states the present and the record was not edited to match it.
...
- [ ] Every considered option carries a disposition; rejections state why, deferrals name a reopening condition.
...
- [ ] Every suppressed or failing test names its case id, every case id resolves to a `KI-<slug>.md` record, and every expected failure uses the strict form.
