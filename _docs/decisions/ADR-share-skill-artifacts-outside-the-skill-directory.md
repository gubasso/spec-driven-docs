...
Enforced by `distribution:a-skill-package-is-self-contained` and `distribution:a-skill-plans-before-it-acts`.

## Consequences

- Good: one gate file serves every skill and both agent families; a fix lands once.
- Good: the installer's transactional machinery covers the new destination unchanged.
- Bad: an agent must follow an absolute path outside its skill directory, a hop a sandboxed reader may prompt on.

## Status

Superseded by [ADR-install-a-skill-as-a-self-contained-package](./ADR-install-a-skill-as-a-self-contained-package.md)
