...
Naming an expected outcome in the method is how a scenario stops being a test. The next author reads the expectation, sees it matched, and never runs step 3.
...
- A scenario MUST run from a state the reader can reach: a documented install, a named version, no prior artifacts.
...
A single clean run shows the outcome is possible. Performance rows, race conditions, and anything described as flaky need the count, and the count belongs in the evidence where a reader can weigh it.
