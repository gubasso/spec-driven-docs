...
The fixed word is what makes the column greppable. A reviewer counting how many capabilities are still untested runs one grep; a column of synonyms defeats that and nothing else was gained.
...
One legend serves every table in the document. Repeating it above each one is noise; placing it at the bottom means the first table is read without it.
