...
A table cell cannot hold prose, so every qualification leaves the table by a link. This chapter fixes the two places a link may start, where each one lands, and how many are allowed.
...
A heading anchor is checkable. A markdown linter with relative-link verification resolves the file and the `#fragment` against real headings on disk, so a renamed section breaks the build instead of breaking silently. Footnotes are a renderer extension outside the core table specification, they render differently across sites, and no linter validates where one lands.
