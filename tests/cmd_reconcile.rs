...
/// VERIFIES staging:the-manifest-is-written-last
///
/// A landing writes the target and records it, and it keeps no result of
/// its own anywhere else. What happened is the tree, the record, and the
/// Git diff.
#[test]
fn a_landing_records_the_target_and_keeps_nothing_beside_it() {
    let fixture = Fixture::new();
    fixture
        .cmd()
        .args([
    ...
    let record: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(fixture.path().join(".spec-driven-docs/manifest.json")).unwrap(),
    )
    .unwrap();
    assert_eq!(record["canon_version"], env!("CARGO_PKG_VERSION"));
    assert_eq!(record["profile"], "codebase");

    for kept in ["spec-driven-docs/plans", "spec-driven-docs/results"] {
        assert!(
            !fixture.state_root().join(kept).exists(),
            "the landing kept {kept} beside the target"
        );
    }
}
